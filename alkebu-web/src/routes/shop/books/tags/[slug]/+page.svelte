<script>
    ...
    const baseURL = $derived(`/shop/books/tags/${encodeURI(slug)}/`);
    ...
    const metadata = $derived.by(() => data.seo || {
		title: `Tag: ${tag?.name || 'Books'} ${thisPage}| Alkebu-Lan Images`,
        description: `${tag?.name || 'Tagged'} books`,
		image: metaImg,
		imageAlt: 'bookshelf',
        url: metaURL,
    });
...
<section class="page-header">
    <div class="container">
        <h2><small>Tag:</small><br>{tag?.name || 'Books'}</h2>
